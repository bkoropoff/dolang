...
use dolang_vfs::{
    Vfs as _,
    security::{
        Acl as VfsAnyAcl, AclKind as VfsAclKind, Nfs4Ace as VfsNfs4Ace,
...
/// Do-facing ACL value (`security.unix.Acl` or `security.nfs4.Acl`).
...
/// [`VfsAclKind::Nfs4`]. `nil` yields `None`, since there is nothing to infer
/// from; callers fall back to a `kind:` keyword argument in that case.
pub(crate) fn acl_from_value<'v, 's>(
    ...
    Err(Error::type_error(
        strand,
        "expected security.unix.Acl, security.nfs4.Acl, or nil",
    ))
}

/// Parses the `kind:` keyword argument (`:POSIX:`/`:NFS4:`), defaulting to
/// [`VfsAclKind::Posix`] when absent.
pub(crate) fn acl_kind_sym<'v, 's>(
    ...
    let sym = slot
        .as_sym(strand)
        .ok_or_else(|| Error::type_error(strand, "kind: expected :POSIX: or :NFS4:"))?;
    if sym == global.syms.posix {
        Ok(VfsAclKind::Posix)
    } else if sym == global.syms.nfs4 {
        Ok(VfsAclKind::Nfs4)
    } else {
        Err(Error::value(strand, "kind: expected :POSIX: or :NFS4:"))
    }
}
