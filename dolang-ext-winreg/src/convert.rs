...
///   `Str` -> `Sz`, an iterable of `Str` -> `MultiSz`, `Bool` -> `Dword`,
///   `Int` -> `Dword` if it fits in 32 bits else `Qword`, `Bin` -> `Binary`,
///   `Nil` -> `None`.
...
async fn default_from_do<'v, 's>(
    ...
    match input.view(strand.vm()) {
        ...
        _ => Err(Error::type_error(
            strand,
            "expected Str, iterable of Str, Bool, Int, Bin, or Nil",
        )),
    }
}
...
fn expect_u32<'v, 's>(strand: &mut Strand<'v, 's>, input: Slot<'v, '_>) -> Result<'v, 's, u32> {
    match input.view(strand.vm()) {
        View::Bool(value) => Ok(u32::from(value)),
        View::Int(i) => {
...
fn expect_u64<'v, 's>(strand: &mut Strand<'v, 's>, input: Slot<'v, '_>) -> Result<'v, 's, u64> {
    match input.view(strand.vm()) {
        View::Bool(value) => Ok(u64::from(value)),
        View::Int(i) => {
