...
mod live {
    ...
    async fn exercise(vfs: &AnyVfs) {
        ...
        // then fetch and set the DACL back unmodified. Proves
        ...
        // `SERVICE_ALL_ACCESS`, which includes the `READ_CONTROL`/`WRITE_DAC`
        // rights those calls need.
        //
        // Do not include the owner in the descriptor passed to the setter.
        // Even an unchanged owner is treated as an owner assignment, and
        // Windows rejects it with `ERROR_INVALID_OWNER` when that SID is not
        // assignable by the caller's token (as on GitHub-hosted runners).
        //
