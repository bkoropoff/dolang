...
def rust_table(name: str, values: list[tuple[int, str]], value_type: str) -> str:
    ...
    return (
        "#[rustfmt::skip]\n"
        f"pub(crate) static {name}: &[({value_type}, &str)] = &[\n{rows}\n];\n"
    )


def generate(args: argparse.Namespace) -> str:
    linux = parse_errno(args.linux)
    freebsd = parse_errno(args.freebsd)
    macos = parse_errno(args.macos)
    windows = parse_windows(args.windows)
    sources = [
        *((path.name, path) for path in args.linux),
        *((f"freebsd-{path.name}", path) for path in args.freebsd),
        *((path.name, path) for path in args.macos),
        (args.windows.name, args.windows),
    ]
    provenance = "\n".join(
        f"// {name}: sha256:{digest(path)}" for name, path in sources
    )
    return (
        ...
        + "\n"
        + rust_table("FREEBSD_ERRNO", freebsd, "i32")
        + "\n"
...
def main() -> int:
    ...
    parser.add_argument("--freebsd", type=Path, action="append", required=True)
